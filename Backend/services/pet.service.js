...
// ค้นหาฟังก์ชัน handlePlay แล้วแทนที่ด้วยโค้ดนี้:
export function handlePlay(toy = null) {
  applyTimeDecay();
  
  // กำหนดข้อความตอบกลับตามของเล่นที่กด
  let message = "มาเล่นกัน! โยนบอลมาเลยเมี๊ยว~ 🧶😺";
  if (toy === 'mouse') message = "งับๆๆ! ตะปบหนูได้แล้วเมี๊ยว! 🐭⚡";
  else if (toy === 'ball') message = "เตะลูกบอลกลิ้งไปกลิ้งมา สนุกจัง! ⚽🐾";
  else if (toy === 'catToy') message = "ฟึ่บๆๆ! ไม้ตกแมวดุ๊กดิ๊กๆ! 🎣😼";

  updateState({
    action: "play",
    happiness: petState.happiness + 15, // เล่นของเล่นได้ความสุขเยอะขึ้น
